...
void saveSoln(int i, double time, double dTdt, double dTdt_2, double HRR, double dSdT, double TME, double ChE, double ExD, double f_Ex_Des, double f_En_Conv, double f_Fu_Conv, double lambda, Cantera::vector_fp u_SenCoe, const G& gas, A& soln)
{
    soln(0,i) = time;
    ...
    gas.getMoleFractions(&soln(nsp+14,i));
}

template<class G, class A>
void saveSoln(double time, double dTdt, double dTdt_2, double HRR, double dSdT, double TME, double ChE, double ExD, double f_Ex_Des, double f_En_Conv, double f_Fu_Conv, double lambda, Cantera::vector_fp u_SenCoe, const G& gas, A& soln)
{
    ...
    for (int k = 0; k < nsp; k++) {
        soln(nsp+14+k,back) = gas.moleFraction(k);
    }
}
